...
update = \dict, key, alter ->
    # TODO: after the get, only the value can change.
    # This could be make a lot faster by getting the correct index and then just updatining it.
    # As it is currently setup, this will search for the correct index twice.
    # when findKey dict key is
    #     Ok {value, bucketIndex} ->
    #         when alter (Present value) is
    #             Present newValue ->
    #                 # Known bucketIndex insert (just List.set data)
    #                 insert dict key newValue
    #             Missing ->
    #                 # Known bucketIndex remove (go straight to swap and remove)
    #                 remove dict key
    #     Err KeyNotFound ->
    #         when alter Missing is
    #             Present value ->
    #                 #Known missing key insert. (go straight to trying to insert at index)
    #                 insert dict key value
    #             Missing ->
    #                 dict
    possibleValue =
...
findKey : Dict k v, k -> Result { bucketIndex : Nat, value : v } [KeyNotFound]
findKey = \@Dict { buckets, data, shifts }, key ->
    if !(List.isEmpty data) then
        ...
        findKeyFirstUnroll buckets bucketIndex distAndFingerprint data key
    else
        Err KeyNotFound

findKeyFirstUnroll : List Bucket, Nat, U32, List (k, v), k -> Result { bucketIndex : Nat, value : v } [KeyNotFound] where k implements Eq
findKeyFirstUnroll = \buckets, bucketIndex, distAndFingerprint, data, key ->
    ...
    if distAndFingerprint == bucket.distAndFingerprint then
        ...
        if foundKey == key then
            Ok { bucketIndex: bucketIndex, value }
        else
            findKeySecondUnroll buckets (nextBucketIndex bucketIndex (List.len buckets)) (incrementDist distAndFingerprint) data key
    else
        findKeySecondUnroll buckets (nextBucketIndex bucketIndex (List.len buckets)) (incrementDist distAndFingerprint) data key

findKeySecondUnroll : List Bucket, Nat, U32, List (k, v), k -> Result { bucketIndex : Nat, value : v } [KeyNotFound] where k implements Eq
findKeySecondUnroll = \buckets, bucketIndex, distAndFingerprint, data, key ->
    bucket = listGetUnsafe buckets (Num.toNat bucketIndex)
    if distAndFingerprint == bucket.distAndFingerprint then
        (foundKey, value) = listGetUnsafe data (Num.toNat bucket.dataIndex)
        if foundKey == key then
            ...
            findKeyHelper buckets (nextBucketIndex bucketIndex (List.len buckets)) (incrementDist distAndFingerprint) data key
    else
        findKeyHelper buckets (nextBucketIndex bucketIndex (List.len buckets)) (incrementDist distAndFingerprint) data key

findKeyHelper : List Bucket, Nat, U32, List (k, v), k -> Result { bucketIndex : Nat, value : v } [KeyNotFound] where k implements Eq
findKeyHelper = \buckets, bucketIndex, distAndFingerprint, data, key ->
    bucket = listGetUnsafe buckets (Num.toNat bucketIndex)
    if distAndFingerprint == bucket.distAndFingerprint then
        (foundKey, value) = listGetUnsafe data (Num.toNat bucket.dataIndex)
        if foundKey == key then
...
fillBucketsFromData = \buckets0, data, shifts ->
    ...
    (distAndFingerprint, bucketIndex) = nextWhileLess buckets1 key shifts
    placeAndShiftUp buckets1 { distAndFingerprint, dataIndex: Num.toU32 dataIndex } bucketIndex

nextWhileLess : List Bucket, k, U8 -> (U32, Nat) where k implements Hash & Eq
nextWhileLess = \buckets, key, shifts ->
    hash = hashKey key
    distAndFingerprint = distAndFingerprintFromHash hash
    bucketIndex = bucketIndexFromHash hash shifts

    nextWhileLessHelper buckets distAndFingerprint bucketIndex

nextWhileLessHelper = \buckets, distAndFingerprint, bucketIndex ->
    loaded = listGetUnsafe buckets (Num.toNat bucketIndex)
    if distAndFingerprint < loaded.distAndFingerprint then
        nextWhileLessHelper buckets (incrementDist distAndFingerprint) (nextBucketIndex bucketIndex (List.len buckets))
    else
        (distAndFingerprint, bucketIndex)
